...
# 一个epoch的训练
def train_one_epoch(unet, vision_encoder, train_loader, optimizer, criterion, scaler):
    loss_epoch = 0.
    for step, pair in enumerate(train_loader):
        ...
        i=0
        vae_out = diffusion.q_sample(vision_out, noise_step.long().to(device), eps=noise)
        for time in range(noise_step):
            time = noise_step - time - 1
        #for time in scheduler.timesteps:
            # 准备模型输入：给“带躁”图像加上时间步信息                      
        # 预测噪声分布
        # print('text out', text_out.shape)
            with autocast():
                noise_pred = unet(vae_out, mask_out, time.long().to(device))
        # 降噪
        #pred_noise = pred_noise[0] + 7.5 * (pred_noise[1] - pred_noise[0])
        # 继续添加噪声
                vae_out = diffusion.p_sample(vae_out, noise_pred,time.long().to(device))         
...
for epoch in range(epochs):
    epoch_loss = train_one_epoch(unet, vision_encoder, loader, optimizer, criterion, scaler)   
