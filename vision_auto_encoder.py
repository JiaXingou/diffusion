...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.norm = nn.GroupNorm(num_channels=embed_dim, num_groups=32, eps=1e-6, affine=True)
...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            #middle
            nn.Sequential(ResNetBlock(256, 256), 
                                SelfAttention(256), 
                                ResNetBlock(256, 256)),
            #up
            nn.Sequential(
            ...
            #out
            nn.Sequential(
                nn.GroupNorm(num_channels=64, num_groups=32, eps=1e-6),
                nn.SiLU(),
