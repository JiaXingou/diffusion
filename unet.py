...
class ResNetBlock(nn.Module):
    def __init__(self, dim_in, dim_out, time_emb_dim=320):
        super().__init__()
...
class TransformerBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.dim = dim
        #in
        self.norm_in = nn.GroupNorm(num_groups=16,
                                    num_channels=dim,
                                    eps=1e-6,
                                    affine=True)
        self.cnn_in = nn.Conv2d(dim,
                                ...
                                stride=1,
                                padding=0)
        self.norm_in_2 = nn.GroupNorm(num_groups=16,
                                    num_channels=80,
                                    eps=1e-6,
                                    affine=True)
        self.cnn_in_2 = nn.Conv2d(80,
                                80,
                                kernel_size=1,
                                stride=1,
                                padding=0)
        ...
        self.atten2 = CrossAttention(dim, 80)
        #act
    ...
    def forward(self, q, kv):
        ...
        kv = kv.permute(0, 2, 3, 1).reshape(b2, h2 * w2, 80)
...
class UNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
        #in
        self.in_vae = torch.nn.Conv2d(1, 80, kernel_size=3, padding=1)
        self.in_txt = torch.nn.Conv2d(16, 80, kernel_size=3, padding=1)
        ...
        #down
        self.down_block1 = DownBlock(80, 80)
        self.down_block2 = DownBlock(80, 160)
        self.down_block3 = DownBlock(160, 320)

        self.down_res1 = ResNetBlock(320, 320)
        self.down_res2 = ResNetBlock(320, 320)
        #mid
        self.mid_res1 = ResNetBlock(320, 320)
        self.mid_trans = TransformerBlock(320)
        self.mid_res2 = ResNetBlock(320, 320)
        #up
        self.up_res1 = ResNetBlock(640, 320)
        self.up_res2 = ResNetBlock(640, 320)
        self.up_res3 = ResNetBlock(640, 320)
        ...
        self.up_block1 = UpBlock(160, 320, 320, True)
        self.up_block2 = UpBlock(80, 160, 320, True)
        self.up_block3 = UpBlock(80, 80, 160, False)
        #out
        self.out = torch.nn.Sequential(
            torch.nn.GroupNorm(num_channels=80, num_groups=16, eps=1e-5),
            torch.nn.SiLU(),
            torch.nn.Conv2d(80, 1, kernel_size=3, padding=1),
        )
